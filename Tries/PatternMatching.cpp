...
class Trie {
    ...
    bool search(TrieNode *root, string word) {
        ...
        int index = word[0] - 'a';
        TrieNode *child = NULL;
        // Check if the first character of the current word exists in the trie
        if(root->children[index] != NULL) {
            child = root->children[index];
        }
        else {
    ...
    bool patternMatching(vector<string> vect, string pattern) {
        // Insert all suffixes of all words in the trie
        for(int i=0; i<vect.size(); i++) {
            string currWord = vect[i];
        ...
        // Search for the required pattern in all the suffixes generated
